    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f_customrendertexture IN) : SV_Target
            {
                ...
                if (id < _Udon_StartID || id > _Udon_EndID || id >= _Udon_MaxBodies)
                    return old_vel_misc;
                ...
                if (current_event == EVENT_DEAD)
                {
                    ...
                    for (int i = 0; i < _Udon_MaxBodies; i++)
                    {
                        if (i == id) continue;
                    ...
                    if (selected_target != -1 && rand_val < prob)
                    {
                        ...
                        if (target_event == EVENT_TEAR)
                        {
                            ...
                            float esc_speed = sqrt(2.0 * _Udon_GravitationalConstant * target_pos_mass.w / max(0.1, target_radius));
                            float3 tear_dir = dot(target_dir, target_dir) > 0.000001 ? normalize(target_dir) : float3(0, 1, 0);
                        ...
                        else 
                        {
                            ...
                            float3 rand_vec = float3(hash(seed + 1u) - 0.5, hash(seed + 2u) - 0.5, hash(seed + 3u) - 0.5);
                ...
                for (int i = 0; i < _Udon_MaxBodies; i++)
                {
                    ...
                    float other_radius = GetRadius(other_mass, _Udon_InnerDensity, _Udon_OuterDensity, _Udon_InnerRatio);
                    ...
                    if (dist < sum_outer_radii)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                float overlap_ratio = min(1.0, overlap_vol / max(0.001, FOUR_THIRDS_PI * my_radius * my_radius * my_radius));
                    ...
                    else if (dist < sum_outer_radii * ROCHE_LIMIT_FACTOR && mass >= _Udon_MinInteractMass && other_mass >= _Udon_MinInteractMass)
                    {
                        // 6. （撕裂）如果超过洛希极限
